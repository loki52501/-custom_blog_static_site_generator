...
int main(int argc, char* argv[]) {
    ...
    if (fs::exists(blogDir)) {
        ...
        if (blogTree) {
            ...
            processContentNode(blogTree, "docs", templateContent, parser, emptyNav, cache, newCache, templateHash, true);
    ...
    for (const auto& post : blogsToGenerate) {
        ...
        std::string finalHtml = applyTemplate(templateContent, post.title, post.content, pages, depth, tocHtml, true);
